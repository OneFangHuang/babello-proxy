...
test("the origin always sees one User-Agent, so ad splicing cannot vary the file", async () => {
  ...
  assert.equal(upstream.calls[0]?.headers.get("user-agent"), "babello/1.0");
});
...
test("an enclosure the origin would only call a binary file still gets through", async () => {
  // What a great many podcast CDNs answer for an mp3. Refusing it failed the import
  // outright; the bytes are re-labelled downstream whatever the origin called them.
  for (const type of ["application/octet-stream", "binary/octet-stream"]) {
    const upstream = fakeFetch(new Response("MP3BYTES", { headers: { "content-type": type } }));
    const response = await handle(ask("url=https://host.example/ep.mp3"), upstream);
