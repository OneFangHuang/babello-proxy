 ...
 *
 * `octet-stream` has to be on the list even though it names no format: a great many
 * podcast CDNs answer it for a plain mp3 enclosure, and refusing it failed the import
 * before a byte was read — a worse failure than the playback one this app already
 * fixed, and from the same cause. It does widen this from "media only" to "any binary
 * file", which is what ALLOWED_HOSTS and the key are for. Nothing downstream trusts
 * the declaration in any case; `audioType` re-labels the bytes on arrival.
 */
const ALLOWED_TYPE =
  /^(audio\/|video\/|application\/(rss|atom)?\+?xml|text\/xml|(application|binary)\/octet-stream)/i;

/** Fixed, so the origin's answer never depends on who is asking. See below. */
const UA = "babello/1.0";
